...
# Templates
default_data = {
    'appname': 'EhospitalApp',
    'no_header_pages': ['signup_page','profile_page','signin_page','otp_page','book-appointment']
}
def index(request):
    default_data['current_page'] = 'signup_page'
    return render (request,"app_admin/signup_page.html",default_data)

def signin_page(request):
    default_data['current_page'] = 'signin_page'
    return render(request,"app_admin/signin_page.html",default_data)

def dashboard_page(request):
    default_data['current_page'] = 'dashboard_page'
    return render (request,"app_admin/dashboard_page.html",default_data)
...
def profile_page(request):
    ...
    return render(request,"app_admin/profile_page.html",default_data)

def otp_page(request):
    default_data['current_page'] = 'otp_page'
    return render(request,"app_admin/otp_page.html",default_data)
...
def appointment_book(request):
    ...
    appointment.Full_Name = request.POST['full_name']
    appointment.Email = request.POST['email']
    appointment.Phone_Number = request.POST['phone_number']
    appointment.Gender = request.POST['gender']
    appointment.Book_Date = request.POST['date']
    appointment.Service = request.POST['service'] 
    
    Book_Date = request.POST['date'].split('-')
    appointment.Book_Date = '-'.join(Book_Date)
    appointment.save()      
    return redirect(profile_page)
